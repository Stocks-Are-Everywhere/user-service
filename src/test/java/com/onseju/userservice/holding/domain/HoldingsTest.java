...
class HoldingsTest {
	...
	@Test
	@DisplayName("지정가 매수 시 보유 주식 수량과 총 매수 금액이 업데이트 되어야 한다")
	void updateHoldings_whenLimitBuy_shouldUpdateQuantityAndTotalPurchasePrice() {
		// Given
	...
	@Test
	@DisplayName("시장가 매수 시 보유 주식 수량과 총 매수 금액이 업데이트 되어야 한다")
	void updateHoldings_whenMarketBuy_shouldUpdateQuantityAndTotalPurchasePrice() {
		// Given
		BigDecimal updatePrice = BigDecimal.valueOf(100);
		BigDecimal updateQuantity = BigDecimal.valueOf(10);

		// When
		holdings.updateHoldings(Type.MARKET_BUY, updatePrice, updateQuantity);
	...
	@Test
	@DisplayName("지정가 매도 시 보유 주식 수량과 총 매수 금액이 업데이트 되어야 한다")
	void updateHoldings_whenLimitSell_shouldUpdateQuantityAndTotalPurchasePrice() {
		// Given
	...
	@Test
	@DisplayName("시장가 매도 시 보유 주식 수량과 총 매수 금액이 업데이트 되어야 한다")
	void updateHoldings_whenMarketSell_shouldUpdateQuantityAndTotalPurchasePrice() {
		// Given
		BigDecimal updateQuantity = BigDecimal.valueOf(1);
		BigDecimal originalTotalQuantity = holdings.getTotalPurchasePrice();

		// When
		holdings.updateHoldings(Type.MARKET_SELL, BigDecimal.ZERO, updateQuantity);
